...
(function () {
    ...
    // Drop your files into /assets/audio/ and keep these entries in sync,
    // OR use the admin panel at /admin/ to upload tracks directly.
    const STATIC_TRACKS = [
    ...
    // Merge with admin-uploaded tracks from localStorage (set via /admin/dashboard.html)
    function buildPlaylist() {
        try {
            const raw = localStorage.getItem('acn-tracks');
            if (raw) {
                const uploaded = JSON.parse(raw);
                if (uploaded && uploaded.length) {
                    return uploaded.map(t => ({
                        title: t.title || 'Untitled',
                        artist: t.artist || 'Unknown',
                        src: t.dataUrl,   // base64 data URL
                    }));
                }
            }
        } catch (_) { }
    ...
    function renderPlaylist() {
        ...
        PLAYLIST.forEach((t, i) => {
            ...
            li.innerHTML = `<span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${t.title}</span><span style="font-size:9px;color:var(--text-dim);">${t.artist}</span>`;
