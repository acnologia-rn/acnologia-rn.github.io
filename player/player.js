...
(function () {
    ...
    function animateEQ() {
        ...
        bars.forEach(bar => {
            const h = isPlaying ? (Math.random() * 16 + 2) : 2;
            bar.style.height = h + 'px';
        });
        eqRaf = requestAnimationFrame(animateEQ);
    }

    // Start/stop EQ with playback
    const origPlay = play;
    play = function () { origPlay(); if (!eqRaf) animateEQ(); };
    const origPause = pause;
    pause = function () { origPause(); };
